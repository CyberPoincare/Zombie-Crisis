...
### 广播通知系统与民防撤离 (Broadcast Announcement & Civil Evacuation)

- **全局广播任务 (Broadcast Task Implementation)**:
    - **零成本零冷却**: 引入了全新的“广播通知” (📢) 工具，允许指挥官随时指定撤离点。
    - **动态无线电播报**: 实现了基于市政局 (City Council) 名义的全局无线电广播，动态融合当前爆发地点与撤离点名称。
    - **立体音效反馈**: 合成了高低频交替的全局警报器 (Siren) 音效，提升下达任务时的临场感。
- **平民撤离 AI (Civilian Evacuation Logic)**:
    - **引导性移动**: 实现了平民在听到广播后，若处于非恐慌状态，会主动向绿色撤离点 (🏠) 靠拢的逻辑。
    - **视觉方位提示**: 在地图上新增了带有绿光脉动的撤离点标记，方便玩家实时微调撤离路线。

### 游戏难度与战备阶段增强 (Difficulty & Readiness Enhancement)

- **战备限制机制 (Readiness Constraint)**:
    - **爆发前禁令**: 为了模拟真实危机爆发初期的混乱与决策延迟，在首个僵尸被发现前，所有进攻/支援性任务（空投、特种兵、空袭、医疗）均处于禁用状态。
    - **紧急准备周期**: 僵尸发现后，总部会下达“紧急战备”指令，各部门进入独立的准备倒计时（需 20-60 秒不等），增加前期生存压力。
- **状态同步优化**:
    - **数据流一致性**: 修复了 `App.tsx` 与 `GameMap.tsx` 之间关于 `infectionDetected` 状态同步可能滞后的问题。

## [1.4.2] - 2025-12-29

### 高级感知系统与动态行为 (Advanced Perception System & Dynamic Behavior)

- **多维感官系统实现 (Multi-Sensory Implementation)**:
  - **视觉方位化**: 引入了 120 度人类视觉扇区，使侧翼包抄和背后接近成为可能。
  - **建筑物理遮挡**: 实现了视觉感知受建筑外形阻隔的多维逻辑，室内外视角完全隔离。
  - **特异性感知**:
    - **僵尸嗅觉**: 实现了 500 米超长距离嗅觉，支持僵尸在无视视觉遮挡的情况下追踪人类气味。
    - **群体听觉检测**: 实现了穿墙听觉，平民可凭声响感知建筑外密集（>3个）的僵尸群。
    - **专业人员战术视野**: 士兵获得基于卫星/通讯的高精尖探测能力，极大提升了远程接敌效率。

### 智能行为逻辑与数值平衡 (AI Logic & Numerical Balancing)

- **平民行为树深化 (Civilian Behavior Refinement)**:
  - **室内偏好逻辑**: 实现了平民 ~95% 的建筑内初始化分布，并增加了 5% “流浪者”偏好设定。
  - **基于感知的恐慌触发**: 弃用了距离触发，改为基于视觉看到僵尸后触发恐慌计时器（10秒）。
  - **寻找掩护行为**: 实现了恐慌状态下且视线内无僵尸时，平民会主动奔向并进入最近建筑进行躲避的逻辑。
  - **心境与对话增强**: 新增了“室内待机”与“建筑内躲藏”心声池，同步更新了配套的心情图标（🏠, 🏢, 🚪, 😊, 😱, 😨, 💨）。
- **感染者行为重构 (Zombie Behavior Refinement)**:
  - **变频移动系统**: 实现了“平时游荡、战时冲刺”的双速系统，游荡速度极低，进入追击范围后大幅提速。
  - **动态地形惩罚**: 僵尸在建筑内移动时会受到显著的速度衰减，为平民争取躲避空间。
- **环境可视化与数值缩放 (Environment & Scaling)**:
  - **爆发源视觉标记**: 在爆发源建筑物上新增了高频脉动的“☣️ 爆发点”动态图标，辅助玩家快速定位威胁起点。
  - **全局比例重调**: 为适配地图空间感知，统一降低了平民、士兵及僵尸的 50% 基准速度，使单位移动更符合地理真实感。
  - **感染阈值平衡**: 缩短了持续感染时间并略微增加感染判定距离，确保降速后威胁依然有效。

### 游戏设定与项目文档体系化 (Game Settings & Documentation Systematization)

- **深度游戏设定文档 (Comprehensive Game Settings)**:
  - **全方位设定汇编**: 创建/丰富了 `doc/game_settings.md`，提供了涵盖角色行为、感知系统、武器数值及任务逻辑的权威指南。
  - **角色逻辑详解**: 详尽描述了平民（包含 5% 流浪者设定）、僵尸（动态感染与嗅觉追踪）、士兵及医疗兵的行为树逻辑。
  - **战术数值公示**: 首次在文档中公开了手枪、散弹枪、狙击枪、火箭筒及喷网枪的射航、伤害及战术特性。
- **项目索引体系增强 (Project Indexing Enhancement)**:
  - **双语 README 联动**: 在中文 (`README.md`) 与英文 (`README_en.md`) 首页同步集成了“游戏设定”入口，极大提升了新开发者和玩家的上手效率。
- **文档规范化修复**: 修正了感知系统数据表在部分 Markdown 渲染器下的对齐问题。
